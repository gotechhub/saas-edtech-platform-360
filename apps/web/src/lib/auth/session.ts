...
export type PortalSession = {
  ...
  portalId: string;
  membershipId: string;
  role: ReturnType<typeof roleFromKeys>;
  roleKeys: string[];
  aal: string | null;
};
export type PortalSessionResult =
  | { kind: "ready"; session: PortalSession }
  | { kind: "unauthenticated" | "mfa_required" | "forbidden" };
...
export async function getPortalSession(industry: string, slug: string): Promise<PortalSessionResult> {
  ...
  return resolvePortalSession(supabase, industry, slug);
}
...
export async function resolvePortalSession(
  ...
  const { data: claimData, error: claimError } = await supabase.auth.getClaims(accessToken);
  ...
  const roleIds = (assignmentResult.data ?? []).map((item) => item.role_id as string);
  const roleResult = roleIds.length
    ? await supabase.from("roles").select("key").eq("tenant_id", tenantId).in("id", roleIds)
    : { data: [], error: null };
  if (roleResult.error) return { kind: "forbidden" };

  const roleKeys = (roleResult.data ?? []).map((item) => item.key as string);
  return {
    kind: "ready",
    session: {
      userId: claims.sub,
      ...
      tenantId,
      portalId: portalResult.data.id as string,
      membershipId,
      role: roleFromKeys(roleKeys),
      roleKeys,
